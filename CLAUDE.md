...
**Stage constant SYNC (4 places):** the stage list is duplicated across `src/constants/pipeline.js` (`PIPELINE_STAGES`, the richer source: columns, colors, tooltips), `src/constants/options.js` (`STAGES`, used by EditOpportunityModal + the metric modals for stage *names*), and the `VALID_STAGES` arrays in `api/opportunities.js` and `api/opportunities/[id].js` (POST/PATCH validation). All four carry `// SYNC: pipeline stages` comments — add/rename/remove a stage in all four or promotes/drags 400. **`opportunities.stage` and `stage_transitions.{from,to}_stage` are now plain `TEXT`** (validation is the app-level `VALID_STAGES`). They were originally Postgres ENUMs, and the value lists kept drifting from the app (`invalid input value for enum stage`); `convertEnumColumnsToText()` in `api/opportunities.js` migrates them to TEXT on deploy (`scripts/pipeline-activation.sql` does the same manually), with `ensureEnumValues()` as a transitional fallback that runs first. So adding a future stage = update the 4 SYNC points only; no enum step. (`project_type` and `outcome` were converted the same way.)
...
**`opportunities.project_type` is now plain `TEXT`** (was a Postgres ENUM that still carried stale legacy values `'research'`, `'senior_design'`, … — see `PROJECT_TYPE_MAP` in `api/opportunities/[id].js`). The enum's values had drifted from the app's display values (`invalid input value for enum project_type`), so `convertEnumColumnsToText()` in `api/opportunities.js` migrates it (and `stage`, `outcome`, the `stage_transitions` stage columns) to TEXT on deploy; `scripts/pipeline-activation.sql` does the same manually. `ensureEnumValues()` / `VALID_PROJECT_TYPES` remain as a transitional pre-conversion fallback (**SYNC:** `VALID_PROJECT_TYPES` ↔ `PROJECT_TYPES` in `src/constants/pipeline.js`). Validation is now the modal/edit dropdowns rather than the DB type.
...
- **`prospect_contacts`** - Structured person-level contacts per prospect (QA audit E5; self-ensured like the other Section E additions, mirrored in `scripts/create-prospect-contacts.sql`)
  ...
  - **Bulk conference-attendee load:** `scripts/load-conference-contacts.mjs <attendee.xlsx> [--apply]` matches an attendee export (First/Last/Organization/Email) to prospects by **exact** normalized company name (no fuzzy), inserts deduped+cleaned contacts (trims emails, strips stray chars/whitespace, dedupes within-file and against existing by email), appends one `psb_connection_notes` line per matched prospect (idempotent via a `PSB Plastics Conference` marker), and recomputes `priority_score`/`ai_readiness` for scored prospects whose notes were empty (the +4 Relationship-Warmth bump) **without touching `updated_at`** — preserving any `priority_manual` override. Dry-run by default; re-runnable as a no-op. The June 2026 PSB Plastics Conference list was loaded this way: **31 contacts across 13 prospects** (mostly equipment/infra partners — Beaumont, Arburg, INCOE, Mold-Masters, PCS — plus converters Flex, Plastikos, PTI, Technimark, Truck-Lite, Chevron Phillips). Attendee files are **not** committed (personal emails). Most conference attendees are PSB/academic/vendor orgs that don't match a prospect and are skipped by design.
...
- **`prospect_companies`** - ~920-company prospect database for alliance outreach (originally seeded at 179; grown via state/conference imports and deduped — see "Data-Quality Maintenance Scripts")
  - `id` (SERIAL, PK)
...
An embedded AI assistant that answers questions, compares prospects, and (Phase 2) **drafts outreach/notifications**, grounded entirely in data it fetches through eight read-only tools. The data path is **strictly read-only** — it **never** edits records, creates tasks, changes priority, or **sends** anything (drafting produces text the user reviews/edits/sends themselves). **Scope (Phase 1):** prospects **and** the live Pipeline **and** state research reports — tools read `prospect_companies` + the ontology + `prospect_attachments` + `opportunities`/`activities` + `state_research_reports`. The system prompt keeps **prospects** (research targets) distinct from **opportunities** (active deals with stage/owner/`waiting_on`). Reachable two ways: contextually from a prospect (ProspectDetail), and **globally from the header "Ask AI" button** (no prospect context → whole-app analyst).

- **Draft mode (Phase 2, L1):** the same `?action=assistant` arm accepts `mode:'draft'` in the body → appends `ASSISTANT_DRAFT_GUIDANCE` to the system prompt (first-touch outreach voice; stakeholder-routing matrix; ground every claim, `[placeholder]` for unknowns, `Subject:` + body, no fabrication). Still read-only on data and **sends nothing**. Entry points: **"Draft"** button in ProspectDetail (seeds a prospect outreach email) and OpportunityDetail (seeds a stakeholder notification, routed by project type). `AssistantModal` gains `mode` + `initialMessage` props (auto-sends the seed once on open) and a per-message **Copy** button. Output is copy-to-clipboard only — no send (sending via Resend-with-approval is a deferred Phase-5 option).
- **Proactive triage (Phase 3, L0/L1):** all on-demand (no change to the unattended `daily-digest` cron — in-email narration was deferred for the ~10s function budget). `CallSheet.jsx` renders its own `AssistantModal` and exposes two seeded launches: a per-call **"Prep"** button (prospect-scoped talking points) and a header **"Brief my day"** button (global → ranks today's priorities across prospects + pipeline). Both surfaces inherit into the Today view, which embeds `CallSheet`.
- **AI ontology extraction (Phase 4a, L1):** `GET ?action=ai-extract-ontology&id=X` (read-only — no write) reads the saved research brief + existing entities, calls Together once (9.5s `AbortController`, `parseExtractionJson` strips fences/prose, validates against `AI_EXTRACT_ENTITY_TYPES`/`AI_EXTRACT_REL_TYPES` — **SYNC** with `ImportOntologyModal`'s `VALID_*` lists), and returns `{entities, relationships}`. The **"Extract with AI"** button in `ImportOntologyModal` feeds that into the existing validate → preview → **import** flow, so the human confirms and the write still goes through the unchanged `import-ontology-extraction` path (`ExtractionPromptModal` copy-paste remains a fallback). Function count stays 10/12. (Phase 4b — AI proposing edits to canonical `prospect_companies` fields — deferred.)
...
- **Runtime:** a server-side tool-use loop against **Together.ai's OpenAI-compatible chat completions API** (`https://api.together.xyz/v1/chat/completions`) via **plain `fetch`** (no SDK — house style, like Resend). Auth is `Authorization: Bearer ${process.env.TOGETHER_AI_API}`. Model `meta-llama/Llama-3.3-70B-Instruct-Turbo` (a **current, stable-id, tool-calling** Together model — the first ship used `deepseek-ai/DeepSeek-V3`, a stale snapshot id Together no longer serves under that exact string, so every call 4xx'd; date-stamped DeepSeek ids like `-V3-0324`/`-V3.1` rotate and re-break, so a stable id is preferred). `max_tokens` 2048, loop capped at 8 turns; single JSON response (no streaming). Tools use OpenAI's `{type:'function', function:{name, description, parameters}}` shape (the neutral tool defs' `input_schema` is mapped to `parameters` at call time); tool calls come back on `choices[0].message.tool_calls` with `finish_reason === 'tool_calls'`, and results go back as `{role:'tool', tool_call_id, content}` messages. All work awaited before `res.json()`. (Was originally built against the Anthropic Messages API; switched to Together.ai — to swap providers again, the change is isolated to this arm's `callModel` + the endpoint/model/auth constants.)
...
- **Nine tools** (module-scope `assistant*` executors + `runAssistantTool` dispatcher; each a `SELECT`-only query mirroring an existing arm — **zero writes anywhere in the path**): `search_prospects` (list arm), `get_prospect` (trimmed export-json bundle), `find_similar_prospects` (`ontology-similar`), `query_ontology` (`ontology-query`), `get_research_brief` (`prospect_attachments`), and **Phase 1 additions** `search_pipeline` (opportunities list — stage/owner/`waiting_on`/`last_activity_at`/`source_prospect_name`), `get_opportunity` (one deal + its last 10 `activities`), `get_state_report` (current `state_research_reports` row, content capped 8K), and `list_companies_by_category` (deterministic category roster — case-insensitive substring match on `category`; with no arg it returns the full category distribution + counts). `ontology-neighborhood` is deliberately NOT a tool (it needs an entity_id, not a prospect_id). **Param convention:** prospect tools use **`prospect_id`** (read `input.prospect_id ?? input.id`); the opportunity tool uses **`opportunity_id`** (reads `?? input.id`); `get_state_report` takes a 2-letter `state`; `list_companies_by_category` takes an optional `category` substring.
- **Category search (the "empty/fabricated knowledge & catalog companies" fix — Brett's report):** `search_prospects` free-text now also matches `category` + `also_known_as`, and its `category` filter is an **assistant-only case-insensitive substring** (NOT `buildCategoryCondition`, whose exact parent-group logic the Prospects-tab filter still depends on — untouched). The system prompt forbids inferring a company's category from outside knowledge and maps Brett's vocabulary: **"knowledge companies" = `Knowledge Sector`, "catalog companies" = `Catalog/Standards`**. `list_companies_by_category` is the authoritative roster for "who are the X companies". **Hybrid tags:** a company can carry a combined category such as `Knowledge AND Catalog` (e.g. Kistler Group); the substring match catches these, and the prompt/tool-description steer the model to query by the **bare type word** ("knowledge", "catalog") — not the full exact category name — so hybrids surface under *each* component type and aren't dropped. Previously a value like `'catalog'` hit `buildCategoryCondition`'s exact-match fallback (`category = 'catalog'` → 0 rows), so the assistant either returned an empty set or padded a broad search with fabricated category labels.
- **Token-bounding:** search ≤50 rows, `query_ontology` ≤25, `find_similar` ≤15, `list_companies_by_category` list mode ≤100 companies (distribution mode returns all ~68 categories — small), brief excerpt 500 chars in `get_prospect` / ~8000 in `get_research_brief`, plus a 12K hard cap per stringified `tool_result`.
- **UI:** `src/components/prospects/AssistantModal.jsx` — a `z-[60]` sub-modal in the `ExportJsonModal` family. **Two launch modes:** (1) **contextual** from the ProspectDetail header "Ask AI" button (`Sparkles`) — passes a `prospect`, sends `prospectId`, prospect-scoped starter prompts; (2) **global** from the **header "Ask AI" button** (`Header.jsx`, no `prospect` prop → `prospectId: null`) — whole-app starter prompts ("What's in the pipeline and what's stalled?"). `prospect` is optional; `isGlobal = !prospect` drives the title/subtitle/placeholder/suggestions. Non-streaming: one `authFetch` POST per turn returns `{ answer, toolsUsed }`; the full visible conversation is re-sent each turn. Answers render through the shared `ReportMarkdownRenderer`; `toolsUsed` shows as plain-English "Based on:" tags (jargon tool names mapped via `TOOL_LABELS`/`TOOL_TIPS`). Read-only framing throughout. The ProspectDetail launch is in its `anySubModalOpen` Escape-stacking guard; the global launch self-manages in Header (own state + bottom render, like DigestPrefs).
...
**PSB Plastics Conference identified stubs (`scripts/add-conference-prospects.mjs`):** Two molders met at the June 2026 PSB Plastics Conference but not previously tracked — **Port Erie Plastics** (Erie, PA) and **Niagara Bottling** (CA HQ) — added as minimal `prospect_status='Identified'` stubs carrying only grounded fields (company, `website` from the attendees' email domain, `source_report='PSB Plastics Conference 2026'`, a PSB-connection note listing who we met, and their conference contacts). `category` / `signal_count` / `priority_score` left **NULL** on purpose — unresearched, no fabricated data, so they enter the research queue honestly (contrast Flex, which had real research to score). Idempotent (skips by exact company name); no ontology rebuild needed (no ontology-bearing fields set).
...
One-off/maintenance scripts run directly against prod via `DATABASE_URL` (no local-dev workflow; Kyle has no clone). **Shared conventions — every data-mutating script follows these:** dry-run by default + `--apply` to write; idempotent (re-runnable as a no-op); **never touch `updated_at`** (staleness detection must not reset); **preserve `priority_manual`** overrides; recompute `priority_score`/`ai_readiness` by importing the canonical pure scorer `src/utils/priorityScore.js` (the server's `rebuild*`/`recalculate*` are NOT exported — don't import them); inline the verified data + per-value source/confidence in the script for the record; print a reviewable plan before writing (the table-review gate — Kyle approves before `--apply`). **ESM `xlsx` gotcha:** `XLSX.readFile` is omitted in the ESM build — use `XLSX.read(readFileSync(path), { type: 'buffer' })`.

- **`scripts/dedup-prospects.mjs`** — removes exact-company-name duplicate rows. **Gotcha it addresses:** two botched bulk imports double-inserted whole batches (a `2026-04-01` NC "Tier 3 Triage" run with a clean **+20 id offset**, and a `2026-04-04` Montana run ~6 min apart), inflating the count. Re-points any real attached data (attachments/tasks/contacts/opportunities/unique activity-log entries) onto the kept row **before** deleting — the `ON DELETE CASCADE` FKs (`prospect_attachments`, `prospect_activity_log`, `prospect_tasks`, `prospect_status_transitions`, `prospect_contacts`) would otherwise wipe it. `ontology_entities` is `ON DELETE SET NULL` (orphans self-heal on next Layer-1 rebuild). **Keep-distinct rule:** same-name rows in different cities/states with their own data are real multi-site or name-collisions (e.g. Watertown Plastics CT vs SD) — NOT merged.
- **`scripts/enrich-converters-from-research.mjs`** / **`scripts/enrich-band-from-research.mjs`** — apply web-researched field fills (`year_founded`, `press_count`, `employees_approx`, `key_certifications`, `medical_device_mfg`, `rjg_cavity_pressure`, `ownership_type`) and recompute scores. **Research-fan-out pattern** (how the band of 55 was gathered): a `Workflow` fans out one read-only research agent per company, each required to **confirm company identity** (plastics names repeat across states) and **cite a source + confidence (high/med/low) or explicit `not_found`** for every value — **zero fabrication**. Use a **shallow** structured-output schema (array of flat `{field, value, confidence, source}` — deep/nested schemas hit the StructuredOutput retry cap). Only **high/medium** findings are written; low-confidence, value conflicts, and most `medical='No'` calls are **held for human review**. `press_count` is rarely public — `not_found` is the expected answer; never infer it from headcount.
- **`scripts/recalc-all-priorities.mjs`** — global priority recompute, mirroring `recalculateAllPriorities()` in `api/prospects.js`. **Maintenance need:** scorer fixes (notably the PE-ownership fix — see Priority Score section) are applied in code but only re-propagate on import/PATCH, so stored scores drift stale (≈92 rows were under-scored, suppressing real targets). Scoped to **already-scored rows** — deliberately does NOT newly-score currently-NULL stubs (e.g. Port Erie, Niagara stay unscored until researched). **`updated_at` untouched; `priority_manual` preserved.**

**Orphaned human HIGH-PRIORITY designations (gotcha):** a company badged `priority='HIGH PRIORITY'` whose computed score caps below 75 (formula-unreachable for small converters) AND whose `priority_manual` is NULL is an *unprotected* human designation — **any** recompute (this script OR the app's next import) silently strips it. Fix per the existing rule (Flex/Strategic-Membership note): a human HIGH must set **both** `priority` AND `priority_manual`. `recalc-all-priorities.mjs` carries a `PRESERVE` map that codifies known cases (e.g. X-Cell Tool & Mold, Group 1 rank 2) by setting `priority_manual` so the badge survives. When manually elevating a company, always set both fields.
