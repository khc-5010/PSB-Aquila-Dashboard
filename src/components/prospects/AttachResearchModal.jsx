...
export default function AttachResearchModal({ prospect, onClose, onSaved }) {
  const { user } = useAuth()
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/prospects?action=attach', {
        method: 'POST',
    ...
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <div>
            <h3 className="text-lg font-semibold text-[#041E42]">Attach Research Brief</h3>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-gray-200">
          <p className="text-xs text-gray-400">
            Status will auto-advance to "Outreach Ready"
          </p>
          <div className="flex gap-3">
            ...
            >
              {saving ? 'Saving...' : 'Save Research Brief'}
            </button>
          </div>
        </div>
      </div>
